...
| `Hud` | Core-frame work recovered direct `Globals` routing, `PlayerEgo::level`, the `Hud::init` image/coordinate map, exact 20-entry event queue, four quick-menu construction modes, menu frame/cargo gauge, status/gamma rows, and linked gamma runtime. Follow-ups now recover steering/fire/secondary/boost/dock/camera/auto-turret controls; cargo, passenger, production and timed-mission panels; volatile cargo fill; four hit-direction pulses; target-context overlay; docking loading/unloading progress and mission markers; jump/cloak charge progress; and the campaign mining-hint pulse. The complete `hudEvent` switch now covers events `1..47`, Android GameText and sound routing, state-only progress events, event `19`'s native Hud string, duplicate suppression, and the `0x100019` important-event mask. A fixed-width `HudArm32Layout` compile-checks the full Android allocation size `0x53c`. Runtime migration step 1 now places the iPad prefix, `menuButtons` and the real 20-element String array at exact ARM offsets through `+0x100`, raising constructor similarity from `5.6%` to `92.7%`. The shared `Globals::hints[0x11]` backing is proven from ELF symbols and `resetHints` no longer contains a null trap shim. `MGame` now passes the native `LevelScript` mission-time expression instead of wall-clock time. `hudAction`, `drawControlsInterface`, `drawBigNumber`, `drawTitleImage`, and `drawCredits` are confirmed Android no-ops. See `HUD_CORE_FRAME_2026-07-20.md`, `HUD_STATUS_GAMMA_BAR_2026-08-16.md`, `HUD_CONTROL_CLUSTER_2026-08-16.md`, `HUD_MISSION_TARGET_OVERLAYS_2026-08-16.md`, `HUD_POST_FIRE_PROGRESS_2026-08-16.md`, `HUD_EVENT_ROUTING_2026-08-16.md`, `HUD_ARM32_ABI_LAYOUT_2026-08-16.md`, and `HUD_RUNTIME_PREFIX_2026-08-16.md`. | Native UCRT64 build is green. Current ARM fuzzy matches are `92.7%` for `Hud::Hud`, `73.3%` for `Hud::~Hud`, `8.1%` for `Hud::draw`, `9.7%` for `Hud::hudEvent`, `77.6%` for `Hud::sameHudEventAsBefore`, and `48.3%` for `Globals::resetHints`; none are byte-equal. Event/progress, image/coordinate and timer/camera runtime regions, String temporary source shape, and the MGame tutorial-completion consumer remain. |
