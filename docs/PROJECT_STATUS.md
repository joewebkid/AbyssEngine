...
| `Hud` | Core-frame work recovered direct `Globals` routing, `PlayerEgo::level`, the `Hud::init` image/coordinate map, exact 20-entry event queue, four quick-menu construction modes, menu frame/cargo gauge, status/gamma rows, and linked gamma runtime. Follow-ups recover the control cluster, cargo/mission overlays, docking/jump/cloak progress and mining hint. The complete `hudEvent` switch covers events `1..47`, Android GameText/sound routing, duplicate suppression and the `0x100019` important-event mask. `HudArm32Layout` compile-checks the full Android allocation size `0x53c`. Runtime migrations now place the prefix, 20-String array, event/queue/touch state, all 71 native Image2D handles and the secondary-label String at exact ARM offsets through `+0x3bf`. Duplicate native handles such as the two `0x53a` and `0x4b1/0x4b0` pairs are preserved. See `HUD_ARM32_ABI_LAYOUT_2026-08-16.md`, `HUD_RUNTIME_EVENT_PROGRESS_2026-08-17.md`, and `HUD_RUNTIME_IMAGE_SPAN_2026-08-17.md`. | UCRT64 is green; ARM corpus is `201/204` with three unrelated known failures. Current matches include `98.8%` constructor, `90.7%` destructor, `9.8%` init, `9.7%` hudEvent and `92.3%` touchEnd. Across 48 Hud functions, average similarity is `59.7%`, with nine linked-exact and eight raw-byte-exact. Coordinate holes and the timer/camera tail remain; the local object is `0x50c` versus native `0x53c`. |
