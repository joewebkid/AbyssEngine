...
| `Level` particle setup | The previously absent player-engine loop in `Level::initParticleSystems` is now source-backed: it creates `field_80` systems for `ParticleSet(29 + nozzle)`, records their handles in `field_a8`, and applies confirmed Android scale, position, color, and per-ship atlas UV data to `ParticleSettingsRef::cur`. `setPlayerEngineColor` now updates those same native slots directly. See `LEVEL_PARTICLE_SYSTEMS_2026-07-21.md`. | Native UCRT64 build is green. The wider `ParticleSettingsRef` runtime/layout and ARM instruction shape remain separate work. |
