...
| `MGame` touch routing | The unpaused `OnTouchEnd` branch consumes `Hud::touchEnd` quick-menu and orbit actions. The paused branch now also recovers autopilot-menu confirmation/cancel and the StarMap close/exit/stream return path, including native landmark-1 orientation and `+8000` positioning. See `MGAME_HUD_ACTION_ROUTING_2026-07-20.md` and `MGAME_PAUSED_TOUCH_ROUTING_2026-07-20.md`. | Generic ChoiceWindow, dialogue/cutscene, game-over, MenuTouchWindow and special modal branches remain separate work; no ARM byte-match claim. |
