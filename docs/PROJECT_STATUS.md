...
| `Hud` | Core-frame work recovered direct `Globals` routing, `PlayerEgo::level`, the `Hud::init` image/coordinate map, exact 20-entry event queue, four quick-menu construction modes, menu frame/cargo gauge, status/gamma rows, and linked gamma runtime. Follow-ups now recover steering/fire/secondary/boost/dock/camera/auto-turret controls; cargo, passenger, production and timed-mission panels; volatile cargo fill; four hit-direction pulses; target-context overlay; docking loading/unloading progress and mission markers; jump/cloak charge progress; and the campaign mining-hint pulse. The shared `Globals::hints[0x11]` backing is proven from ELF symbols and `resetHints` no longer contains a null trap shim. `MGame` now passes the native `LevelScript` mission-time expression instead of wall-clock time. `hudAction`, `drawControlsInterface`, `drawBigNumber`, `drawTitleImage`, and `drawCredits` are confirmed Android no-ops. See `HUD_CORE_FRAME_2026-07-20.md`, `HUD_STATUS_GAMMA_BAR_2026-08-16.md`, `HUD_CONTROL_CLUSTER_2026-08-16.md`, `HUD_MISSION_TARGET_OVERLAYS_2026-08-16.md`, and `HUD_POST_FIRE_PROGRESS_2026-08-16.md`. | Native UCRT64 build is green. Current ARM fuzzy matches are `8.1%` for `Hud::draw`, `6.0%` for `Hud::hudEvent`, and `48.3%` for `Globals::resetHints`; none are byte-equal. Exact 32-bit Hud tail ABI and the MGame tutorial-completion consumer remain. |
