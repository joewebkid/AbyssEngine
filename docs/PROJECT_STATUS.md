...
| `MGame` HUD routing | The unpaused `OnTouchEnd` branch now consumes the action masks emitted by `Hud::touchEnd`: quick-menu navigation, cloak, secondary selection, wingman commands, Khador drive, orbit targets, and docking targets. The `MGame+0xc8` low byte is now named `orbitMenuOpen`. See `MGAME_HUD_ACTION_ROUTING_2026-07-20.md`. | UCRT64 build is green; paused modal/StarMap/cutscene routing and the rest of this large touch handler remain separate work. |
