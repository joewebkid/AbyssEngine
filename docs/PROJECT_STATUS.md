...
| `Hud` | Core-frame work recovered direct `Globals` routing, `PlayerEgo::level`, the `Hud::init` image/coordinate map, exact 20-entry event queue, four quick-menu construction modes, menu frame/cargo gauge, status/gamma rows, and linked gamma runtime. Follow-ups now recover steering/fire/secondary/boost/dock/camera/auto-turret controls; cargo, passenger, production and timed-mission panels; volatile cargo fill; four hit-direction pulses; and the target-context overlay. `MGame` now passes the native `LevelScript` mission-time expression instead of wall-clock time. `hudAction`, `drawControlsInterface`, `drawBigNumber`, `drawTitleImage`, and `drawCredits` are confirmed Android no-ops. See `HUD_CORE_FRAME_2026-07-20.md`, `HUD_STATUS_GAMMA_BAR_2026-08-16.md`, `HUD_CONTROL_CLUSTER_2026-08-16.md`, and `HUD_MISSION_TARGET_OVERLAYS_2026-08-16.md`. | Native UCRT64 build is green. The post-fire docking/jump/cloak/extender progress family and exact 32-bit Hud ABI remain; whole `Hud::draw` ARM fuzzy match is `1.8%`, not byte-equal. |
