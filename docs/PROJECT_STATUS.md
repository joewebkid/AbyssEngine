...
| `MGame` touch routing | The unpaused `OnTouchEnd` branch consumes `Hud::touchEnd` quick-menu and orbit actions. The paused branch now covers autopilot-menu confirmation/cancel, StarMap close/exit/stream return, ordinary ChoiceWindow dismissal, and MenuTouchWindow end routing/cinematic synchronization. See `MGAME_HUD_ACTION_ROUTING_2026-07-20.md` and `MGAME_PAUSED_TOUCH_ROUTING_2026-07-20.md`. | Dialogue/cutscene, game-over, and selector-specific ChoiceWindow branches remain separate work; no ARM byte-match claim. |
