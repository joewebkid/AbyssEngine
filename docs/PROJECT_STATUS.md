...
| --- | --- | --- |
| `MenuTouchWindow` | Draw/touch routing, load/save, settings, HUD layout, DLC/store, and language-list paths received focused source-backed passes. The main-state dispatcher covers the confirmed `+0x04` button ID table, cargo summary, Google Play/restore/quit controls, and native X/Y forwarding. The state/dialog follow-up restores the phone/iPad `state 3` options body, states `10/12/13/17`, the calibration-pending byte at `+0x176`, native challenge fade values, and the selected DLC/game context at `+0x234`. Constructor social controls live in the confirmed shared auxiliary array at `+0xc0`, separate from DLC selector slots at `+0xf8`. See `MENUTOUCH_MAIN_DISPATCH_FREECAM_ARM_2026-08-23.md` and `MENUTOUCH_STATE_DIALOG_OPTIONS_ARM_2026-08-23.md`. Local source-backed coverage remains an estimate around 70%; state-9 return ABI, state-15/16 DLC modals, exact String cleanup and whole-switch shape remain. | UCRT64 is green. Forced-inline `OnTouchEnd` is `3.7%` at `3001/2675` target/base instructions, not linked- or byte-exact; the preceding `7.4%` at `3001/1856` omitted the new state bodies from the function. The source body is closer in instruction span, but still differs in CFG, temporary lifetime and frame shape. `isMakingScreenshot` is `100%` linked- and byte-exact at `6/6`. |
