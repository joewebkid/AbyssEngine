...
| Resource loading | AEM mesh creation/clone paths and the AEI `ResourceTexture -> TextureCreate -> TextureCreateFromFileIntern` path are source-backed. The AEI pass restores typed `AELoadedTexture` ownership, format-specific mip uploads, filters, cubemap faces, cache writes and GL/error accounting. `TextureCreateFromFile` and the callback-free PaintCanvas wrapper are normalized ARM matches. See `AEM_AEI_NATIVE_LOADER_SPEC.md` and `AEI_TEXTURE_UPLOAD_ARM_2026-08-27.md`. | The large internal loader is `30.3%` at `678/730` instructions and the main PaintCanvas resource wrapper is `50.4%`; neither is byte-exact. Real GLES suspend/reload coverage and `ImageCreateFromFile`/region byte matching remain. |
