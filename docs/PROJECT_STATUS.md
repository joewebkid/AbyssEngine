...
| --- | --- | --- |
| `MenuTouchWindow` | Draw/touch routing, load/save, settings, HUD layout, DLC/store, and language-list paths received focused source-backed passes. The main-state dispatcher now covers the confirmed `+0x04` button ID table, cargo summary, Google Play/restore/quit controls, and native X/Y forwarding. Constructor social controls now live in the confirmed shared auxiliary array at `+0xc0`, separate from DLC selector slots at `+0xf8`. See `MENUTOUCH_MAIN_DISPATCH_FREECAM_ARM_2026-08-23.md`. Local source-backed coverage remains an estimate around 65-70%; exact String cleanup and whole-switch shape remain. | UCRT64 is green. `OnTouchEnd` is `7.4%` at `3001/1856` target/base instructions, up from `5.4%` at `3001/1274`; it is neither linked- nor byte-exact. `isMakingScreenshot` is `100%` linked- and byte-exact at `6/6`. |
