...
| `Hud` | Core-frame work recovered direct `Globals` routing, `PlayerEgo::level`, the `Hud::init` image/coordinate map, exact 20-entry event queue, four quick-menu construction modes, menu frame/cargo gauge, status/gamma rows, and linked gamma runtime. Follow-ups recover the control cluster, cargo/mission overlays, docking/jump/cloak progress and mining hint. The complete `hudEvent` switch covers events `1..47`, Android GameText/sound routing, duplicate suppression and the `0x100019` important-event mask. `HudArm32Layout` compile-checks the full Android allocation size `0x53c`. Runtime migrations now place the iPad prefix, `menuButtons`, 20-String array, four event Strings, event queue, touch state and progress flags at exact ARM offsets through `+0x298`. The false standalone `autoTurretFlags` byte was replaced by native touch mask `0x20000000`. Constructor similarity has risen from `5.6%` to `97.6%`. See `HUD_ARM32_ABI_LAYOUT_2026-08-16.md`, `HUD_RUNTIME_PREFIX_2026-08-16.md`, and `HUD_RUNTIME_EVENT_PROGRESS_2026-08-17.md`. | UCRT64 is green; ARM corpus is `201/204` with three unrelated known failures. Current matches include `97.6%` constructor, `88.2%` destructor, `10.3%` init, `9.7%` hudEvent and `92.3%` touchEnd. `releaseAllKeys`, jump-map accessors and `cargoFull` are linked- and byte-exact. Image/coordinate and timer/camera runtime regions, temporary String source shape and the MGame tutorial-completion consumer remain. |
